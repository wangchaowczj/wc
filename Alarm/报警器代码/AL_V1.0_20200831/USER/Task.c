	...
	switch(prio)
	{
        ...
        case APP_TEST_TASK_PRIO:
			Result = OSTaskCreateExt(AppTestTask, (void*)p_arg,
						...
						(void *)0, OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR); 
            break;	
        case APP_POWER_TASK_PRIO:
			Result = OSTaskCreateExt(AppPowerOffTask, (void*)p_arg,
						&PrivateStk.POWEROFF[APP_POWEROFF_TASK_STK_SIZE-1],prio,
						prio,&PrivateStk.POWEROFF[0], APP_POWEROFF_TASK_STK_SIZE,
						(void *)0, OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR); 
            break;
        case APP_QUEUE_TASK_PRIO:
			Result = OSTaskCreateExt(AppQueueTask, (void*)p_arg,
						&PrivateStk.QUEUE[APP_QUEUE_TASK_STK_SIZE-1],prio,
						prio,&PrivateStk.QUEUE[0], APP_QUEUE_TASK_STK_SIZE,
						(void *)0, OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR); 
            break;
		default:
			break;
...
void AppLEDTask(void* p_arg)
{
    while(1)
    {
        WatchDogFeed(); //看门狗放在最低优先级里面
        LED1_TOGGLE();
        OSTimeDly(200);         
    }
}

void AppQueueTask(void* p_arg)
{
    u16 value;
    
    while(1)
    {
        value = EliminateBusCurrentErr(); 
        Queue_push(value); 
        OSTimeDly(1);
    }
}

void AppPowerOffTask(void* p_arg)
{
    while(1)
    {
        AutoPowerOff();
...
void AppTestTask(void* p_arg)
{
    while(1)
    {
        ...
        if((queue.current[queue.front] < 10) && (queue.current [queue.rear] > 20))
        {
            Sound(2);            
        }
