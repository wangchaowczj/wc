...
void OLEDDisplay(void)
{
    ...
    OSTaskSuspend(APP_POWER_TASK_PRIO); 
    ...
    OSTaskResume(APP_POWER_TASK_PRIO); 
...
{
    if(SET == RCC_GetFlagStatus(RCC_FLAG_SFTRST))//查询复位方式
    {
...
//| 修改记录 | 修改人：          时间：         修改内容： 
//==================================================================================================
ErrorStatus Queue_push(u16 data)
{
    if(queue.queue_flag  != 0x01)
    {
        queue.current[queue.rear] = data;	
        queue.rear += 1;
        queue.count += 1;

        if(queue.count == ARRAY_MAX)
        {
            queue.queue_flag = 0x01;	
        }
    }
    else if(queue.queue_flag == 0x01)
    {
        if(queue.count == ARRAY_MAX)
        {
